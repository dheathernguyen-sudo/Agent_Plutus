...
Snapshot JSON format on disk:
{
    ...
    "accounts": {
        ...
        "robinhood": {
            "total_mv": 34106.79,   # gross securities
            "cash": -14134.87,      # negative cash = margin debt
            "net_mv": 19971.92,     # total_mv + cash
            "holdings": {...}
        },
        ...
    },
    "liquid_total_mv": 95000.0,   # everything except merrill_401k
    "total_mv": 120000.0
}

`cash` and `net_mv` are populated for accounts that flow through
`_extract_provider_accounts` (Robinhood, Merrill 401k). Fidelity accounts
do not carry a separate cash field in the current pipeline schema.
"""
...
_ILLIQUID_SUBSTRINGS = ("merrill_401k", "401k")
...
def _extract_fidelity_accounts(fid_data: dict) -> dict:
    ...
    fid_data is keyed by account label such as "fidelity_Z23889908".
    Values are either:
...
def _extract_provider_accounts(raw: dict, provider_label: str) -> dict:
    """Extract normalised accounts from an rh_raw or merrill_raw dict.
    ...
    # Normalise to ticker-keyed dict. SnapTrade/Plaid raw extract has a flat
    # list of {ticker, quantity, institution_price, institution_value, ...};
    # the older pipeline-normalised form is {ticker: {qty, price, mv, ...}}
    # or date-keyed {date: {ticker: ...}}.
    if isinstance(raw_holdings_outer, list):
        flat_holdings = {}
        for h in raw_holdings_outer:
            if not isinstance(h, dict):
                continue
            tkr = h.get("ticker") or h.get("symbol")
            if not tkr:
                continue
            flat_holdings[tkr] = {
                "qty": h.get("quantity", h.get("qty", 0)),
                "price": h.get("institution_price", h.get("price", 0)),
                "mv": h.get("institution_value", h.get("mv", 0)),
            }
    else:
    ...
    # Cash balance (negative = margin debt for Robinhood). Read from either:
    #  - raw SnapTrade format: data["accounts"][i]["balances"]["current"]
    #  - pipeline format:      data["cash"]
    # Absent means the provider didn't report a cash balance.
    cash = 0.0
...
def save_snapshot(
    fid_data: dict,
    rh_raw: dict,
    merrill_raw: Optional[dict] = None,
    ...
    ----------
    fid_data:
        Dict keyed by account label (e.g. ``"fidelity_Z23889908"``).
    ...
    merrill_raw:
        Same structure as *rh_raw*, or ``None``.
    date_str:
    ...
    # --- Merrill 401k ---
    if merrill_raw:
        accounts.update(_extract_provider_accounts(merrill_raw, "merrill_401k"))
