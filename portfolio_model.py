...
def _merge_live_merrill(live_extraction):
    """Extract live 401(k) fund holdings from Plaid Merrill data.

    Returns list of {name, current_value, cost_basis, unrealized_gl, return_pct}
    or empty list if no Merrill data available.
    """
    if not live_extraction:
        return []

    # The pipeline passes raw extraction data. Merrill data may be under
    # various keys depending on how prepare_builder_data processed it.
    # Check for raw Plaid format first.
    merrill_raw = None
    for key in ("merrill", "merrill_lynch"):
        if key in live_extraction and isinstance(live_extraction[key], dict):
            merrill_raw = live_extraction[key]
            break
...
def _merge_live_holdings(acct_data, live_extraction, acct_key):
    ...
    # Match by account-number suffix (last 4 digits) so it works whether SnapTrade
    # returns un-redacted ("Z23889908") or redacted ("*****9908") account numbers.
    suffix_map = {
        "fidelity_brokerage": "9908",
        "fidelity_roth_ira": "9863",
        "fidelity_hsa": "9651",
    }
    candidates = []
    if acct_key in live_extraction:
        candidates.append(acct_key)
    suffix = suffix_map.get(acct_key)
    if suffix:
        for k in live_extraction:
            if isinstance(k, str) and k.startswith("fidelity_") and k.endswith(suffix):
                candidates.append(k)
    for ext_key in candidates:
...
def build_model(data_dir="data/", live_extraction=None, raw_extraction=None, benchmarks=None, cash_data=None, snapshot_dir=None):
    ...
    Args:
        ...
        raw_extraction: full raw extraction dict (includes Merrill Plaid data with securities list)
    ...
    for key, raw in accounts_raw.items():
        ...
        # When holdings come from live extraction, money-market positions
        # (FCASH/SPAXX/FDRXX) already represent cash — don't double-count via
        # the stale cash_position stored in data/*.json.
        cash_position = 0 if holdings_source == "live" else (raw.get("cash_position", 0) or 0)
        ...
        if "quarterly" in raw:
            ...
            live_401k = _merge_live_merrill(raw_extraction)
            acct_model["live_holdings"] = live_401k
            if live_401k:
                live_mv = sum((h.get("current_value") or 0) for h in live_401k)
                live_cb = sum((h.get("cost_basis") or 0) for h in live_401k)
                acct_model["gains"]["total_mv"] = round(live_mv, 2)
                acct_model["gains"]["total_cb"] = round(live_cb, 2)
                acct_model["gains"]["unrealized"] = round(live_mv - live_cb, 2)
                acct_model["returns"]["cb_return"] = _compute_cb_return(acct_model["gains"])
