...
from paths import EXTRACT_OUTPUT
...
def main():
    ...
    sectors = [
        ('Defense Tech', 163254.72, 0.3813, None, None, None, None, None, 163254.72, 2),
        ('Diversified/Index', 83726.58, 0.1956, None, None, 83726.58, None, None, None, 5),
        ('Technology', 74429.77, 0.1739, 20988.03, 8427.75, None, 5152.32, 9861.67, 30000, 17),
        ('Industrials', 24931.17, 0.0582, 11665.05, 3266.12, None, None, None, 10000, 6),
        ('Other', 17676.76, 0.0413, 17676.76, None, None, None, None, None, 1),
        ('Healthcare', 16147.33, 0.0377, None, None, None, None, 6147.33, 10000, 4),
        ('Consumer Discretionary', 13596.95, 0.0318, 7974.78, 4106.03, None, None, 1516.14, None, 5),
        ('Financials', 13525.68, 0.0316, 13525.68, None, None, None, None, None, 2),
        ('Diversified/ETF', 10398.60, 0.0243, None, 7543.54, None, 2855.06, None, None, 3),
        ('Communication Services', 3905.40, 0.0091, 1932.20, None, None, 1973.20, None, None, 2),
        ('Utilities', 3273.84, 0.0076, None, None, None, 3273.84, None, None, 1),
        ('Real Estate', 3231.81, 0.0075, 1414.91, None, None, 1816.90, None, None, 3),
    ]
    ...
    cell(ws, row, 2, 428098.60, font=BOLD_FONT, fmt=DOLLAR)
    cell(ws, row, 3, 1, font=BOLD_FONT, fmt=PCT)
    cell(ws, row, 10, 51, font=BOLD_FONT)
    for col in range(4, 10):
        ws.cell(row=row, column=col).border = THIN_BORDER
    row += 1

    cell(ws, row, 1, 'Top sector: Defense Tech')
    cell(ws, row, 2, 0.3813, fmt=PCT)
    row += 1
    cell(ws, row, 1, 'Top 3 sectors combined')
    cell(ws, row, 2, 0.7508, fmt=PCT)
    row += 2
    ...
    geo = [
        ('United States', 392883.13, 0.9177),
        ('International', 35215.47, 0.0823),
        ('  — Diversified Intl', 10631.81, 0.0248),
        ('  — Canada', 8794.21, 0.0205),
        ('  — Japan (ETF)', 3851.02, 0.0090),
        ('  — South Korea (ETF)', 3692.52, 0.0086),
        ('  — United Kingdom', 2982.20, 0.0070),
        ('  — Netherlands', 2634.46, 0.0062),
        ('  — Taiwan', 2629.25, 0.0061),
    ]
