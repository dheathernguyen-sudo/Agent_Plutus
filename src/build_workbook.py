...
def build_dashboard(wb, model):
    ...
    overview_headers = ['Account', 'Beginning', 'Ending', 'Net Cash Flow',
...
def build_401k_tab(wb, acct, model):
    ...
    # Add provider-stated TWR note (e.g. "Merrill stated: 14.41%")
    twr_stated = acct.get("twr_provider_stated", acct.get("twr_merrill_stated"))
    if twr_stated is not None and "TWR" in row_map:
        twr_r = row_map["TWR"][1]
        provider = acct.get("account", {}).get("provider_name", "Provider")
        _cell(ws, twr_r, 3, f'{provider} stated: {twr_stated:.2%}', font=NOTE_FONT)
