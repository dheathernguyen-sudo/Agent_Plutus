...
def main():
    ...
    data_cell(ws, row, 2, None, fmt=DOLLAR)
    data_cell(ws, row, 3, 'From sold positions (see below)', font=NOTE_FONT)
    row += 1
    ...
    # Holdings data loaded from data/fidelity_brokerage.json
    holdings = [
        ('AAPL', 10, 250.00, 2500.00, 2000.00),
        ('MSFT', 5, 400.00, 2000.00, 1800.00),
        ('VTI', 50, 280.00, 14000.00, 12000.00),
        ('Cash', None, None, 100.00, None),
    ]
    ...
    # Monthly data loaded from data/fidelity_brokerage.json
    # Market Change = Change in Value - Dividends
    monthly_data = {
        'Jan': {'begin': 10000.00, 'add': 500.00, 'sub': 0, 'div': 25.00,
                'change': 200.00, 'end': 10725.00},
        'Feb': {'begin': 10725.00, 'add': 0, 'sub': 0, 'div': 30.00,
                'change': -100.00, 'end': 10655.00},
    }
    ...
    sold_2026 = [
        ('GOOG', 'Jan 2026', 5, 1200.00, 1500.00, 'Full exit'),
    ]
    ...
    data_cell(ws, row, 6, 300.00, font=BOLD_FONT, fmt=DOLLAR)  # Placeholder YTD realized
    for col in [2, 3, 4, 7]:
        ws.cell(row=row, column=col).border = THIN_BORDER
    sold_total_row = row
