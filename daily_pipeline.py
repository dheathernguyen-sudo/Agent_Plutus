...
from paths import PIPELINE_DIR, EXTRACT_OUTPUT
...
def extract_all(start_date, end_date):
    ...
    try:
        # extract_plaid_cash lives in repo/extractors/plaid_extract.py
        import importlib, importlib.util
        _repo_plaid = PROJECT_DIR / "repo" / "extractors" / "plaid_extract.py"
...
def prepare_builder_data(raw, pipeline):
    ...
    build_portfolio.py expects:
      - fid_data: dict keyed by account (e.g. "fidelity_Z23889908") with holdings
...
def run_pipeline(args):
    ...
    if not today_is_trading and not missed and not getattr(args, "force", False):
        logging.info("Not a trading day (weekend or holiday) and no missed days. Skipping.")
        return 0
    if not today_is_trading and getattr(args, "force", False):
        logging.info("--force: running on non-trading day; using cached extraction.")
            ...
            try:
                ...
                if not cash_data:
                    try:
                        ...
                        _rp = PROJECT_DIR / "repo" / "extractors" / "plaid_extract.py"
...
def main():
    ...
    parser.add_argument("--force", action="store_true",
                        help="Run even on non-trading days (weekend/holiday)")
